...
#preprocessing of dataset2, extracting only relevant columns

setwd("C:/Users/yarde/Documents/Bio-Info-Project/second_dataset-ERP000550")

raw_counts <- fread(file = './E-MTAB-567-raw-counts.tsv', stringsAsFactors = T, na.strings = c('NA',''),drop = 'Gene Name')
raw_counts <- raw_counts[complete.cases(raw_counts), ]

cts <- as.matrix(raw_counts, rownames = 'Gene ID')
df_to_compare_2 <- cts

coldata <- fread('./E-MTAB-567-experiment-design.tsv', stringsAsFactors = T, na.strings = c('NA',''))
colnames(coldata)[colnames(coldata) == 'Factor Value[sampling site]'] <- 'condition'
colnames(coldata)[colnames(coldata) == 'Run'] <- 'sample_num'
colnames(coldata)[colnames(coldata) == 'Sample Characteristic[age]'] <- 'patient_age'

coldata <- coldata %>% remove_rownames %>% column_to_rownames(var="sample_num")
coldata <- coldata[,c('condition','patient_age'),drop=FALSE]
...
library(janitor)
compare_df_cols_same(as.data.frame(df_to_compare_2),as.data.frame(cts))
...
#volcano plot
EnhancedVolcano(res,lab = rownames(res), x = 'log2FoldChange',y = 'pvalue',pCutoff = 1e-19, pointSize = 4.0,labSize = 4.0, labCol = 'black',labFace = 'bold', boxedLabels = FALSE, colAlpha = 4/5, legendPosition = 'right', legendLabSize = 14, legendIconSize = 4.0, drawConnectors = TRUE, widthConnectors = 1.0, colConnectors = 'black', title = 'Normal Vs PCa')
...
#removing some samples to show better segregation

expr <- cts
unwanted_samples <- c('ERR031017','ERR031027','ERR031023','ERR031039','ERR031035')
samples_to_take <- setdiff(colnames(expr),unwanted_samples)
expr <- expr[,samples_to_take]
expr <- t(scale(na.omit(expr)))
rownames(expr) <- rename_data[rownames(expr),]
d <- dist(expr, method = "euclidean")
km.result2 <- kmeans(d, centers = 2, nstart = 25)
fviz_cluster(km.result2, data = expr)
```
